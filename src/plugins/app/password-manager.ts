...
export async function scryptHash(value: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    scrypt(
      ...
      function (error, key) {
        /* c8 ignore start - Requires too many configs */
        if (error !== null) {
          reject(error);
        } /* c8 ignore end */ else {
...
async function compare(value: string, hash: string): Promise<boolean> {
  ...
  return new Promise((resolve) => {
    scrypt(
      ...
      function (error, key) {
        /* c8 ignore start - Requires impractical, complex configs */
        if (error !== null) {
...
const passwordManagerPlugin: FastifyPluginAsync = async (fastify) => {
  fastify.decorate('passwordManager', passwordManager);
};

export default fp(passwordManagerPlugin, {
