...
- Ensure schemas match shape of database tables
- Update authentication to use JWT and password hashes are stored in DB

## Database

- Upgrade database to PostgreSQL + Knex (?)
- Add columns:
  - `users.dateOfCreation`
  - `users.dateOfLastAccess`
  - `notes.dateOfCreation`
  - `notes.dateOfLastUpdate`
  - `users.passwordHash` replaces `users.password`

## Test

- Add following planned `user` tests:
...
- Update lint/formatting tools from `eslint` and `prettier` to modern alternative (i.e., `biome` or `oxlint`, `oxformat`)
- Create enum for allowed `scope`s inside of commit messages (not needed yet, actually)
